...
export default function VoiceCallContainer({ data }) {
    ...
    <div className="border-conversation-border overflow-y-auto border-l w-full bg-conversation-panel-background flex flex-col h-[100vh] items-center justify-center text-white p-5">
      <div className="flex flex-col gap-3 items-center py-5">
        ...
        <span className="text-lg">
          ...
          {!isStreamed && callAccepted && localStream && data.type === "out-going" && (
            <div className="bg-green-600 flex items-center p-2 justify-center rounded-xl">
              <button onClick={sendStreams} className="cursor-pointer text-lg">Click to Connect</button>
            </div>
          )}
